...
def remove_openclaw_group_edge_mention(content, to_user_nickname):
    if not isinstance(content, str):
        return content
    nickname = str(to_user_nickname).strip()
    if nickname == '':
        return content
...
def handle_chat_message_with_config(config, model_config, vendor, msg, preset, lcExt, presetExt, preset_name, command_ext, retry_times):
    ...
    if 'ai_generate' in lcExt and lcExt['ai_generate'] == False and not openclaw_direct_command:
        if msg_type == 'CHAT':
            ...
            addHistory(redis, historyListKey, history)
        return ''
    if (content == '/reset_prompt' or content == "/reset") and not openclaw_direct_command:
        if msg_type == 'CHAT':
            removeAllHistory(redis, historyListKey)
        del_preset_name(redis, fromUserId, toUserId)
        del_embedding_info(redis, fromUserId, toUserId)
    ...
    if len(preset_embedding_infos) > 0 and not openclaw_direct_command:
        context = ""
...
def configure_embedding(app_id, embedding_name, admin_user_ids, preset_name, embedding_max_tokens, embedding_max_blocks, embedding_content, new_embedding_name, max_block_size, overlapping_size, vendor, model, tags):
    return lanying_embedding.configure_embedding(app_id, embedding_name, admin_user_ids, preset_name, embedding_max_tokens, embedding_max_blocks, embedding_content, new_embedding_name, max_block_size, overlapping_size, vendor, model, tags)
...
@bp.route("/service/openai/create_vendor", methods=["POST"])
def create_vendor():
    if not check_access_token_valid():
        resp = make_response({'code':401, 'message':'bad authorization'})
        return resp
    ...
    vendor_type = str(data['vendor_type']).strip()
...
@bp.route("/service/openai/configure_vendor", methods=["POST"])
...
def get_vendor_list():
    ...
    data = json.loads(text)
    app_id = str(data['app_id'])
    result = lanying_vendor.get_vendor_list(app_id)
    if result['result'] == 'error':
        resp = make_response({'code':400, 'message':result['message']})
    else:
        resp = make_response({'code':200, 'data':result["data"]})
