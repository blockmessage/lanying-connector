...
def chat(prepare_info, preset):
    ...
    try:
        logging.info(f"ppinfra chat_completion start | preset={preset}, url:{url}")
        logging.info(f"ppinfra chat_completion final_preset: \n{json.dumps(final_preset, ensure_ascii=False, indent = 2)}")
        stream = final_preset.get("stream", False)
        if stream:
            response = requests.request("POST", url, headers=headers, json=final_preset, stream=True)
            logging.info(f"ppinfra chat_completion finish | code={response.status_code}, stream:{stream}")
            ...
            logging.info(f"ppinfra chat_completion finish | code={response.status_code}, response={response.text}")
            res = response.json()
            if response.status_code != 200:
                return {
                    'result': 'error',
            ...
            if 'tool_calls' in response_message and isinstance(response_message['tool_calls'], list) and len(response_message['tool_calls']) > 0:
                try:
                    ...
                    logging.error(f"fail to parse ppinfra function call {response_message}")
                    logging.exception(e)
            finish_reason = ''
            try:
