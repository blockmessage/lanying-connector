...
def get_message_daily_quota_limit_reached(appId):
    default = "抱歉，当前应用今日智能消息额度已达上限，请联系管理员或稍后再试。"
    if mode == 'etcd':
        return get_config_field(appId, 'lanying_connector', 'lanying_connector_message_daily_quota_limit_reached', default)
    return os.getenv('LANYING_CONNECTOR_MESSAGE_DAILY_QUOTA_LIMIT_REACHED', default)
